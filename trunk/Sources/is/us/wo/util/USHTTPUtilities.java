...
public class USHTTPUtilities {
	...
	*/
	...
	/**
	* Creates a WOResponse containing the given data and headers.
	*/
	public static WOResponse responseWithDataAndMimeTypeAndHeaders( String filename, NSData data, String mimeType, NSArray<String> headers ) {
		...
		for ( String header : headers ) {
			if ( header.startsWith( "Expires" ) )
				response.setHeader( header, HEADER_EXPIRES );
			if ( header.startsWith( "Cache-Control" ) )
				response.setHeader( header, HEADER_CACHE_CONTROL );
			if ( header.startsWith( "Pragma" ) )
				response.setHeader( header, HEADER_PRAGMA );
			if ( header.startsWith( "Last-Modified" ) )
				response.setHeader( header, HEADER_LAST_MODIFIED );
		}
